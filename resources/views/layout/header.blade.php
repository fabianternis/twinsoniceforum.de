...
<head>
    ...
    <title>{{ $pageTitle ?? 'Twins on Ice Community Forum' }}</title>
            ...
            <div class="nav-links">
                ...
                @auth
                    ...
                    <div style="display: flex; align-items: center; gap: 0.75rem; margin-left: 0.5rem;">
                        <a href="/user/{{ urlencode(Auth::user()->username) }}" style="display: flex; align-items: center; gap: 0.5rem;">
                            <img src="{{ Auth::user()->avatar_url }}" alt="Avatar" class="avatar-sm">
                            <span style="font-weight: 600; color: #fff; font-size: 0.9rem;">{{ Auth::user()->username }}</span>
                        </a>
